...
// Функция вычисления длины массива доступных для контента платформ:
  function quantity(txt) {
    let regexp = /\[ (\"\w+\"\,( \"\w+\"\,?)*) \]/;   // Формируем шаблон регулярного выражения c двумя скоб. группами
    txt = txt.match(regexp);                          // Производим поиск по шаблону
    let qnt  = txt[1].split(',').length;              // Берём содержимое скобочной группы, создаём массив и считаем количество элементов в нём
    return qnt;
}
