...
Scenario('Get promocode for ivi.ru: ', async ({ I }) => {
  ...
  */
  ...
  // Выполним функцию поиска стартовой позиции массива поддерживаемых платформ, узнаем и введём его длину:
    ...
    I.fillField('input[id="count"]' ,quantity(txt.slice(start-25,start+100)));
    I.click('input[type="submit"]');
...
// Функция вычисления длины массива доступных для контента платформ:
  function quantity(txt) {
    let regexp = /\[ (\"\w+\"\, )+(\"\w+\" \])*/gi; // Формируем шаблон регулярного выражения
    txt = txt.match(regexp).join(''); // Производим поиск по шаблону и сразу преобразуем полученный массив в строку
    txt = txt.replace(/\[/,"").replace(/\]/,""); // Убираем "[" и "]" из полученной выше строки
    let qnt  = txt.split(',').length; // Преобразуем строку вида ' "a", "b", ..., "z" ' в массив и считаем количество элементов
    return qnt;
}
